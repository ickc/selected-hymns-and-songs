...
class TableTest(TestCase):
    ...
    def test_a_row_carries_both_credits(self) -> None:
        with TemporaryDirectory() as name:
            ...
            self.assertEqual(read_authors(path), {1: ("Charles Wesley", "Carl G. Glaser")})
    ...
    def test_a_blank_cell_is_kept_rather_than_dropped(self) -> None:
        ...
        with TemporaryDirectory() as name:
            path = Path(name) / "authors.tsv"
            path.write_text(table("1\t\tJohn Zundel", "2\tIsaac Watts\t", "3\t\t"))

            self.assertEqual(
                read_authors(path),
                {1: ("", "John Zundel"), 2: ("Isaac Watts", ""), 3: ("", "")},
            )

    def test_a_row_that_is_not_three_fields_is_an_error(self) -> None:
        with TemporaryDirectory() as name:
            path = Path(name) / "authors.tsv"
            path.write_text(table("1\tCharles Wesley"))
    ...
    def test_what_is_written_reads_back_the_same(self) -> None:
        with TemporaryDirectory() as name:
            path = Path(name) / "authors.tsv"
            credits = {1: ("Charles Wesley", "Carl G. Glaser"), 2: ("", "John Zundel")}
...
class CreditTest(TestCase):
    ...
    def test_a_hymn_the_table_skips_loses_the_credits_it_had(self) -> None:
        ...
        was = credited(Hymn.from_markdown(HYMN), ("Charles Wesley", "Carl G. Glaser"))
...
class ApplyTest(TestCase):
    ...
    def test_a_row_naming_a_hymn_that_is_not_there_is_an_error(self) -> None:
        ...
        with TemporaryDirectory() as name:
            ...
            with self.assertRaises(ValueError):
                rewrites(files, {1: ("Charles Wesley", ""), 900: ("Someone", "")})
