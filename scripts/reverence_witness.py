...
`data/` carries 你 everywhere -- 4,546 times in 557 files -- because the
...
each hymn's Chinese lines against that layer and says, for every 你 in `data/`,
which of the two the page prints, or that the layer cannot say.

What it is not is an authority.  The layer is a reading like any other -- D17
already found it wrong about 爲 and 裏 more often than not -- so its verdicts
were checked against the page images by sample, its refusals are read off the
images one by one, and its disagreements with the English half of the line
(``Thou`` against a plain 你) are listed for the same treatment.  Every row it
writes carries the page and the height to crop, because that reading is the
point of the exercise and the layer is only what makes it finite.

Usage::

    python scripts/reverence_witness.py --layer ../../private/selected-hymns-and-songs-pdf

The layer lives outside this repository, which is why this is a one-time
extraction whose *output* is committed rather than a check CI can run.
"""
...
# The four shapes of *nǐ*.  `data/` spells the reverential one 你 today and 妳
# eight times where the hymnal addresses a woman or the Church; the layer
# spells the reverential one 妳 and has no 祢 anywhere.  Folding all four is
# what lets an alignment see through the very disagreement it is asked about.
PRONOUNS = "你妳祢袮"
# What a layer character says the page prints.
REVERENTIAL = "妳祢袮"
...
@dataclass(frozen=True)
class Occurrence:
    """One 你 in `data/`, and what the page's text layer says about it."""
    ...
    def conflict(self) -> bool:
        """Say whether the English half contradicts a plain verdict.

        Only this direction is a conflict: the English half saying ``Thou``
        where the layer reads a plain 你.  The other direction -- a modern
        *you* against a printed 祢 -- happens hundreds of times and means
        ...
        return self.verdict == "plain" and bool(THOU.search(self.en))
...
def layer_characters(directory: Path, pages: list[int]) -> list[Character]:
    ...
    for page in pages:
        document = json.loads((directory / f"{page:03d}.json").read_text(encoding="utf-8"))
    ...
    for page, y, x, text in ordered:
        for step, character in enumerate(text):
            if _is_han(character):
                characters.append(Character(character, page, x + step * 9.0, y))
    return characters
...
def hymn_characters(hymn: Hymn) -> list[tuple[str, str, int, int]]:
    ...
    and the page does not set it in the lyric line, so it is dropped here and
    settled by eye.
    """
    ...
    for stanza in hymn.stanzas:
        for index, line in enumerate(stanza.lines):
            text = GLOSS.sub("", line.translations.get("zh", ""))
...
def witness(number: int, hymn: Hymn, layer: list[Character]) -> list[Occurrence]:
    """Say what the page's text layer reads at every 你 of one hymn."""
    ...
    for index, (character, stanza, line, offset) in enumerate(characters):
        if character != "你":
            continue
        seen = read.get(index)
        if seen is None or seen.text not in PRONOUNS:
            verdict, place = "unresolved", _nearest(characters, read, index)
        else:
            verdict = "reverential" if seen.text in REVERENTIAL else "plain"
            place = seen
        zh, en = halves[(stanza, line)]
        occurrences.append(Occurrence(
            number, stanza, line, offset, verdict, zh, en,
...
def _nearest(
    ...
    An unresolved 你 has no position of its own -- the layer did not read it --
    so what is offered instead is the band its line is printed in, taken from
    the nearest character of the *same* line the layer did read.  Falling back
    to another line would name a page band the character is not in, which is
...
def main() -> None:
    ...
    for path in numbered_markdown_files(arguments.data):
        ...
        hymn = Hymn.from_markdown(path.read_text())
        if not any("你" in line.translations.get("zh", "")
                   for stanza in hymn.stanzas for line in stanza.lines):
            continue
        occurrences.extend(
            witness(number, hymn, layer_characters(directory, edition.pages(number)))
        )
    ...
    if arguments.out:
        with arguments.out.open("w", encoding="utf-8", newline="") as handle:
            ...
            writer.writerow((
                "hymn", "stanza", "line", "offset", "verdict", "conflict",
                "page", "top", "zh", "en",
            ))
            for occurrence in occurrences:
                writer.writerow((
                    occurrence.number, occurrence.stanza, occurrence.line,
                    occurrence.offset, occurrence.verdict,
                    "thou" if occurrence.conflict else "",
    ...
    counts = {"reverential": 0, "plain": 0, "unresolved": 0}
    for occurrence in occurrences:
        counts[occurrence.verdict] += 1
    ...
    print(f"{len(occurrences)} occurrences of 你 in {len(hymns)} hymns")
    for verdict, count in counts.items():
        print(f"{count:>6}  {verdict}")
    print(f"{len(unresolved):>6}  hymns with an unresolved occurrence")
    print(f"{unplaced:>6}  unresolved with no band to crop")
    print(f"{conflicts:>6}  plain against an English Thou")
